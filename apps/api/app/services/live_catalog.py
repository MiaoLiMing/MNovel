...
class LiveCatalogService:
    ...
    async def _gutenberg(self, query: str) -> list[ContentSummary]:
        async with httpx.AsyncClient(
            ...
            for entry in root.findall("a:entry", ATOM):
                ...
                if re.fullmatch(
                    r"https://www\.gutenberg\.org/ebooks/\d+\.opds", item_id
                ):
    ...
    def _parse_gutenberg_detail(self, payload: bytes) -> ContentSummary | None:
        ...
        return ContentSummary(
            ...
            cover=cover,
            popularity=(
                f"{int(downloads_match.group(1)):,} 次下载"
                if downloads_match
                else "公共领域"
            ),
    ...
    async def _internet_archive(
        ...
        return [
            self._archive_item(doc, channel) for doc in docs if doc.get("identifier")
        ]
    ...
    def _archive_item(self, doc: dict[str, Any], channel: Channel) -> ContentSummary:
        ...
        if isinstance(description, list):
            description = (
                description[0] if description else "Internet Archive 公开馆藏视频。"
            )
        return ContentSummary(
