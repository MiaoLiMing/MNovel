...
- [ ] 修复裸 IP HTTPS 证书域名不匹配
- [ ] 跨章加载失败时回滚原章节末页并显示友好错误
- [ ] 删除没有真实适配器的商业书源占位项
- [ ] 为内置来源提供能力说明，为自定义来源提供可点击编辑
- [ ] 将三个公开内置来源适配器迁入 unified_backend
- [ ] 补齐统一后端 MNovel API 契约、缓存、超时与失败隔离
- [ ] 执行 Flutter、后端测试和 APK 构建
