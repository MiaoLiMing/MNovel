# 多书源兼容与聚合搜索交付说明（2026-07-30）

## 本轮结果

- 从用户提供的 6 个订阅地址中识别 5 个有效书源数组，接收 1,833 条规则；与 APK 旧目录合并后共 2,247 个唯一来源。
- App 搜索不再随机抽取 12 个内置源，而是调用生产后端聚合接口。
- 聚合器按新/旧、基础/脚本四类规则交错调度，单源失败不会中断查询；成功来源会按成功率和延迟自动前置。
- 单个来源最多占 20 条结果，避免一个站点填满列表后掩盖其他来源。
- 兼容层新增当前 Legado 常用的请求 JSON、POST、GBK、Header、模板表达式、混合大小写 JSONPath、`@put:` 与 URL 字段模板。
- 异常清退、离页持续检测和书源管理页重构按用户要求没有修改。

## 生产验证

- 发布版本：`/opt/madmin/releases/20260730114114`。
- `红楼梦`：41 条，3 个来源（鬼吹灯、溜达小说、扎堆小说）。
- `斗破苍穹`：40 条，2 个来源（鬼吹灯、溜达小说）。
- `诡秘之主`：40 条，2 个来源（鬼吹灯、溜达小说）。
- 最终冷启动实测：`诡秘之主` 约 5.8 秒、`斗破苍穹` 约 2.4 秒；两个来源返回的前三条样本均与关键词相关。
- 已确认 MIUI 旧搜索 API 当前返回 `offline api`，因此不把它伪报为可用搜索源。

## 验证记录

- JVM：Maven 测试通过，远端 Docker 构建再次运行测试通过。
- 后端：全量 Pytest 58 项通过；本次 MNovel 文件 Ruff 通过。全仓 Ruff 仍有其他旧模块既有的 80 项格式问题。
- Flutter：Analyze 通过，全量 44 项测试通过。
...
`D:\喵喵\AIproject\unified_backend\app\api\v1\mnovel` 已成为生产书源解析入口：
...
- 从 `D:\喵喵\AIproject\base.apk` 可复现提取 `assets/bookSource.json`：
  - 原始记录 1,142 条；
...
## 2026-07-30 搜索阅读链路修复与健康书源目录

### 用户侧改动

- 搜索结果使用服务端目录预检得到的真实章节数；不可读或待验证条目显示原因并禁止进入详情。
- 详情加载失败时展示真实错误，收藏、下载和开始阅读保持禁用；空目录不再被视为成功详情。
- 阅读器首次加载失败后保持稳定错误态，不再因布局回调自动重复请求和闪烁；404/Not Found 统一转换为中文提示。
- “我的书源”只显示服务端全链路巡检通过的健康目录，网络失败时回退最近一次成功缓存。
- 书源页移除分类、内置/APK 分组和启停开关，统一显示默认启用的连续列表；全部检测由服务端后台执行，离开页面后继续运行。

### 生产巡检

- 候选总数：2,247；健康：2；隔离：2,245。
- 失败阶段：搜索 1,935、静态规则 278、目录 25、正文 7。
- 健康来源：米读看书[api]、绾书文学。
- 抽样验证：米读看书目录 415/415 章、首章 54 段；绾书文学目录 100/100 章、首章正文可读。

### 验证与产物

- `dart analyze lib test`：通过，`No issues found`。
- Flutter 全量测试：`46 passed`。
- Flutter Web Release：构建成功，并通过 1280x720 桌面与 390x844 手机视口验收，无分类、分组、开关、溢出或交互重叠。
- Android Release：构建成功，78,337,461 bytes（74.71 MiB）。
- APK SHA-256：`A330DA243A86468B9EA08F175F192E28F6B4F5E668827B479C9DD64803DF0594`。
- Windows 中文路径通过项目级路径检查配置和临时 ASCII 盘符完成 AOT 构建；Gradle 仓库保留国内镜像优先、官方仓库回退。

## 2026-07-31 健康书源精简复检与原生源展示

### 用户侧改动

- “全部检测”不再发送 `force=true`，以后只重检服务端当前发布的健康来源。
- 健康目录按服务端 `kind` 区分原生规则和 Legacy 规则；免费小说之王可作为原生内置来源正常展示和搜索。
- 书源页进度总数使用健康池实际数量，不再显示 2,247 条已隔离原始规则。

### 本地验证

- `dart analyze lib test`：通过，`No issues found`。
- Flutter 全量测试：`47 passed`。
- 新增请求契约测试，确认复检接口不携带全目录强制参数。
