...
## 已实施：健康书源永久收敛与原生规则源补录（2026-07-31）

- [x] 将免费小说之王纳入统一健康目录
- [x] 将复检候选池收敛为当前健康快照
- [x] 禁止已隔离规则再次检测或参与搜索
- [x] 统一原生规则与 Legacy 搜索可用集合
- [x] 修改 App 全部检测请求语义和状态展示
- [x] 补充后端与 Flutter 回归测试
- [x] 部署并验证生产复检总数为 14
- [x] 更新文档、提交并推送两个仓库
- [x] 2026-08-01 搜索首屏性能与 biquge.tw 书源接入：完成后端共享预算、Flutter 请求隔离、受限元数据源及后端回归测试（Flutter 验证受旧版 SDK 阻塞）
